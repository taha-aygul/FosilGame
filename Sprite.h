...
inline void Sprite::UpdateFrame()
{
	...
	{
		// Frame tetikleyicisini sýfýrla
		m_iFrameTrigger = m_iFrameDelay;

		// Varsayýlan animasyon aralýðý
		int iStartFrame = 0;
		...
		{
			// Eðer tek döngülük bir animasyon ise sprite'ý yok et
			if (m_bOneCycle)
				m_bDying = TRUE;
			else
				// Mevcut animasyonun baþýna dön
...
{
	// Frame sayýsýný ve döngü ayarýný yap
	m_iNumFrames = iNumFrames;
	m_bOneCycle = bOneCycle;
	m_bFlippable = flippable;
	// Pozisyonu yeniden hesapla
	RECT rect = GetPosition();
	// Sprite'ýn geniþliði artýk tek bir karenin geniþliðidir.
...
{
	// Sadece "flippable" olarak iþaretlenmiþ spritelarýn yönü deðiþtirilebilir.
	if (!m_bFlippable)
		return;

	// Sadece durum deðiþiyorsa iþlem yap
	if (m_bFlipped != bFlipped)
	{
		m_bFlipped = bFlipped;

		// Mevcut frame'i yeni animasyonun baþlangýç karesine ayarla
